...
/// <summary>
/// A place to keep non-fatal warnings, distinct from Errors. Used for things like
/// GamesaveTool.SetTeamString's DefaultJersey range check, where an out-of-range index
/// is still written (matching BadAL's nfl2k5tool_dart SetTeamString behavior exactly)
/// but the user should be told the value falls outside that team's own valid jersey
/// range. Ported from BadAL's nfl2k5tool_dart lib/static_utils.dart (Warnings/AddWarning/
/// ShowWarnings), added alongside the Team Data editing feature.
/// </summary>
public static List<string> Warnings = new List<string>();

/// <summary>
/// Shows the warnings (if any exist). Same presentation pattern as ShowErrors, but with
/// a Warning icon/title instead of Error, and its own independent list so warnings never
/// get mixed into or swallowed by the error dialog/console output.
/// </summary>
/// <param name="showToConsole">true to print the warnings to the console, 
/// false to show them in a GUI dialog</param>
public static void ShowWarnings(bool showToConsole)
{
if (Warnings.Count > 0)
{
StringBuilder b = new StringBuilder();
...
if (!showToConsole)
{
MessageForm form = new MessageForm(System.Drawing.SystemIcons.Warning);
form.Text = "Warning!";
form.MessageText = b.ToString();
form.ShowDialog();
}
...
/// <summary>
/// Add a warning to the session. Unlike AddError, a warning means the value was still
/// written -- it's advisory only, never a rejected write.
/// </summary>
...
/// </summary>
/// <param name="target">The bytes to look for</param>
/// <param name="data">The data to search through.</param>
/// <param name="start">where to start in 'data'</param>
/// <param name="end">Where to end in 'data'</param>
/// <returns>a list of addresses</returns>
