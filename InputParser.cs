...
{
Tool.SetPlayerContractAttribute(player, (ContractDetails)attr, attribute);
}
...
string desc;
if (attr >= AttributeRanges.ContractDetailsStart && attr <= AttributeRanges.ContractDetailsEnd)
desc = ((ContractDetails)attr).ToString();
else if (attr >= AttributeRanges.AppearanceAttributesStart && attr <= AttributeRanges.AppearanceAttributesEnd)
desc = ((AppearanceAttributes)attr).ToString();
else
desc = ((PlayerOffsets)attr).ToString();
